...
The three robots in the factory move to process the parts
	...
	def create(self):
		...
		pick2_group = 'robot2'
		robot2_loc = Pose2D(x= -4.3, y= -0.9, theta=0)
		gripper2 = "vacuum_gripper2_suction_cup"
		# x:29 y:187, x:594 y:345
		...
		with _state_machine:
			...
			# x:90 y:430
			OperatableStateMachine.add('Move R2 back to Home_1',
			...
			OperatableStateMachine.add('Move R2 to pick',
										...
										transitions={'reached': 'Move R2 back to Home_1', 'planning_failed': 'failed', 'control_failed': 'failed'},
										autonomy={'reached': Autonomy.Off, 'planning_failed': Autonomy.Off, 'control_failed': Autonomy.Off},
										remapping={'joint_values': 'pick1_configuration', 'joint_names': 'joint_names'})
